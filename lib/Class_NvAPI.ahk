...
; ###############################################################################################################################
    ...
    GetAssociatedDisplayOutputId()
    {
        ...
        if !(NvStatus := DllCall(NvAPI._GetAssociatedDisplayOutputId, "Ptr", hNvDisplay, "UInt*", pOutputId))
            return pOutputId
        return "*" NvStatus
    }
...
; ###############################################################################################################################
    ...
    SetDVCLevelEx(version, currentLevel, minLevel, maxLevel, defaultLevel, outputId := 0)
    {
        static NV_DISPLAY_DVC_INFO_EX := 20
        hNvDisplay := NvAPI.EnumNvidiaDisplayHandle()
        VarSetCapacity(pDVCInfo, NV_DISPLAY_DVC_INFO_EX), NumPut(version, pDVCInfo, 0, "UInt")
        , NumPut(currentLevel, pDVCInfo, 4, "Int"), NumPut(minLevel, pDVCInfo, 8, "Int")
        , NumPut(maxLevel, pDVCInfo, 12, "Int"), NumPut(defaultLevel, pDVCInfo, 16, "Int")
